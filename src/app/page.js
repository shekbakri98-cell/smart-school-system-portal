    ...
    <div className="min-h-screen bg-[#0f172a] text-slate-100 p-6">
      ...
      <main className="max-w-6xl mx-auto">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div className="bg-[#1e293b] p-5 rounded-xl border border-slate-700 flex flex-col justify-between shadow-xl">
                ...
                <div className="mt-4 pt-2 border-t border-slate-700">
                  ...
                  <svg className="w-full h-1.5 bg-[#0f172a] rounded-full overflow-hidden"><rect x="0" y="0" width="74%" height="100%" fill="#10b981" /></svg>
                </div>
              </div>
              <div className="bg-[#1e293b] p-5 rounded-xl border border-slate-700 flex flex-col justify-between shadow-xl">
                <div>
                ...
                <div className="mt-4 pt-2 border-t border-slate-700">
                  ...
                  <svg className="w-full h-1.5 bg-[#0f172a] rounded-full overflow-hidden"><rect x="0" y="0" width="78.4%" height="100%" fill="#3b82f6" /></svg>
                </div>
              </div>
              <div className="bg-[#1e293b] p-5 rounded-xl border border-slate-700 flex flex-col justify-between shadow-xl">
                <div>
                ...
                <div className="mt-4 pt-2 border-t border-slate-700">
                  ...
                  <svg className="w-full h-1.5 bg-[#0f172a] rounded-full overflow-hidden"><rect x="0" y="0" width="92.1%" height="100%" fill="#f59e0b" /></svg>
                </div>
              </div>
            </div>
          </div>
            ...
            <section className="bg-[#1e293b] p-4 rounded-lg border border-slate-800 space-y-3">
              ...
              {userRole === 'Admin' ? (
                <form onSubmit={handleFinanceSubmit} className="space-y-2">
                  <input type="text" placeholder="ID Barataa" value={financeForm.studentId} onChange={e => setFinanceForm({...financeForm, studentId: e.target.value})} className="w-full bg-[#0f172a] border border-slate-800 p-2 text-white outline-none rounded" required />
                  <select value={financeForm.feeType} onChange={e => setFinanceForm({...financeForm, feeType: e.target.value})} className="w-full bg-[#0f172a] border border-slate-800 p-2 text-white outline-none rounded">
                    <option value="Tuition Q1">Kaffaltii Kurmaana 1ffaa</option>
                    <option value="Tuition Q2">Kaffaltii Kurmaana 2ffaa</option>
                  </select>
                  <input type="number" placeholder="Total Due" value={financeForm.amountDue} onChange={e => setFinanceForm({...financeForm, amountDue: e.target.value})} className="w-full bg-[#0f172a] border border-slate-800 p-2 text-white outline-none rounded" required />
                  <input type="number" placeholder="Amount Paid" value={financeForm.amountPaid} onChange={e => setFinanceForm({...financeForm, amountPaid: e.target.value})} className="w-full bg-[#0f172a] border border-slate-800 p-2 text-white outline-none rounded" required />
                  <button type="submit" className="w-full bg-purple-600 p-2 text-white font-bold rounded uppercase">Commit Dues</button>
                </form>
              ) : (
                <div className="p-4 text-center text-red-400 bg-[#0f172a] border border-red-900/30 rounded-lg">⚠️ Akeekkachiisa: Uunka kaffaltii herregaa jijjiiruuf aangoo Daayirektaraa qabaachuu si barbaachisa.</div>
              )}
            </section>
            <section className="lg:col-span-2 bg-[#1e293b] p-4 rounded-lg border border-slate-800">
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-left">
                  <thead>
                    <tr className="text-slate-400 text-[10px] uppercase"><th className="pb-2">ID Barataa</th><th>Maqaa</th><th>Ramaddii</th><th>Idaa</th><th>Kaffalame</th><th className="text-right">Haala Kaffaltii</th></tr>
                  </thead>
                  <tbody className="divide-y divide-slate-800 text-slate-300">
                    {financeLedger.map((f, idx) => (
                      <tr key={idx} className="hover:bg-slate-900/20">
                        <td className="py-2.5 font-bold text-purple-400">{f.studentId}</td><td>{f.name}</td><td>{f.fee_type}</td><td>{f.amount_due}</td><td>{f.amount_paid}</td>
                        <td className="text-right">
                          <span className={`px-2 py-0.5 rounded text-[10px] font-bold border \${f.payment_status === 'Paid' ? 'border-emerald-800 text-emerald-400' : 'border-amber-800 text-amber-400'}`}>{f.payment_status === 'Paid' ? 'Kaffalameera' : 'Hanga Tokko'}</span>
                        </td>
                      </tr>
                    ))}
                  </tbody>
        ...
        {activeTab === 'director-users' && (
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 font-mono text-xs">
            <section className="bg-[#1e293b] p-4 rounded-lg border border-slate-800 h-fit space-y-3">
              ...
              {userRole === 'Admin' ? (
                <form onSubmit={handleUserCreationSubmit} className="space-y-2">
                  <input type="text" placeholder="Maqaa Fayyadamaa" value={userForm.username} onChange={e => setUserForm({...userForm, username: e.target.value})} className="w-full bg-[#0f172a] border border-slate-800 p-2 text-white outline-none rounded" required />
                  <input type="email" placeholder="Imeelii" value={userForm.email} onChange={e => setUserForm({...userForm, email: e.target.value})} className="w-full bg-[#0f172a] border border-slate-800 p-2 text-white outline-none rounded" required />
                  <input type="password" placeholder="Jecha Iccitii" value={userForm.password} onChange={e => setUserForm({...userForm, password: e.target.value})} className="w-full bg-[#0f172a] border border-slate-800 p-2 text-white outline-none rounded" required />
                  <select value={userForm.role} onChange={e => setUserForm({...userForm, role: e.target.value})} className="w-full bg-[#0f172a] border border-slate-800 p-2 text-white outline-none rounded"><option value="Teacher">Barsiisaa</option><option value="Admin">Daayirektara (Admin)</option></select>
                  <button type="submit" className="w-full bg-purple-600 p-2 text-white font-bold rounded uppercase">Galmeessi</button>
                </form>
              ) : (
                <div className="p-4 text-center text-red-400 bg-[#0f172a] border border-red-900/30 rounded-lg">⚠️ Akeekkachiisa: Uunka kana fayyadamuuf aangoo Daayirektaraa qabaachuu si barbaachisa.</div>
              )}
            </section>
            <section className="lg:col-span-2 bg-[#1e293b] p-4 rounded-lg border border-slate-800">
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-left">
                  <thead>
                    <tr className="text-slate-400 text-[10px] uppercase"><th className="pb-2">Maqaa Eenyummaa</th><th>Karaa Imeelii</th><th className="text-right">Aangoo Hayyamaa</th></tr>
                  </thead>
                  <tbody className="divide-y divide-slate-800 text-slate-300">
                    {systemUsers.map((user, idx) => (
                      <tr key={idx} className="hover:bg-slate-900/40"><td className="py-2.5 font-semibold text-slate-200">{user.username}</td><td>{user.email}</td><td className="text-right font-bold text-purple-400">{user.role === 'Admin' ? 'Bulchaa' : 'Barsiisaa'}</td></tr>
                    ))}
                  </tbody>
                </table>
              </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 font-mono text-xs">
            <section className="bg-[#1e293b] p-4 rounded-lg border border-slate-700 h-fit space-y-3 shadow-xl">
              ...
              <form onSubmit={handleEnrollmentSubmit} className="space-y-2">
                <select value={selectedGrade} onChange={(e) => setSelectedGrade(e.target.value)} className="w-full bg-[#0f172a] border border-slate-700 p-2 text-white outline-none rounded"><option value="12 Natural">12 Natural</option><option value="12 Social">12 Social</option></select>
            ...
            <section className="lg:col-span-2 bg-[#1e293b] p-4 rounded-lg border border-slate-700 overflow-hidden shadow-xl">
              <div className="flex justify-between items-center mb-4">
                ...
                <select value={selectedGrade} onChange={(e) => setSelectedGrade(e.target.value)} className="bg-[#0f172a] border border-slate-700 p-1 text-white rounded outline-none text-xs"><option value="12 Natural">12 Natural</option><option value="12 Social">12 Social</option></select>
              </div>
              <div className="overflow-x-auto">
                <table className="w-full text-left whitespace-nowrap">
                  <thead>
                    <tr className="border-b border-slate-700 text-slate-400 text-[10px] uppercase tracking-wider"><th className="pb-2">ID</th><th>Maqaa Barataa</th><th>T1 (10)</th><th>T2 (10)</th><th>Asgn (20)</th><th>Final (60)</th><th className="text-center">Waliigala</th><th className="text-right">Waraqa</th></tr>
                  </thead>
                  <tbody className="divide-y divide-slate-700 text-slate-300">
                    {students.map((s, idx) => (
                      <tr key={idx} className="hover:bg-[#0f172a]/40 transition-colors">
                        <td className="py-2.5 font-bold text-blue-400">{s.studentId}</td><td className="font-semibold text-slate-100">{s.name}</td>
          ...
          <section className="bg-[#1e293b] p-4 rounded-lg border border-slate-800 text-xs font-mono w-full">
            <div className="flex justify-between items-center border-b border-slate-800 pb-2 mb-3">
              ...
              <div className="flex gap-2 text-white">
                <select value={selectedGrade} onChange={(e) => setSelectedGrade(e.target.value)} className="bg-[#0f172a] border border-slate-800 p-1 rounded text-xs"><option value="12 Natural">12 Natural</option><option value="12 Social">12 Social</option></select>
                <input type="date" value={attendanceDate} onChange={(e) => setAttendanceDate(e.target.value)} className="bg-[#0f172a] border border-slate-800 p-1 rounded text-xs" />
              </div>
            </div>
            <div className="overflow-x-auto">
              <table className="w-full text-left whitespace-nowrap">
                <thead>
                  <tr className="border-b border-slate-800 text-slate-400 uppercase text-[10px]"><th className="pb-2">ID Barataa</th><th>Maqaa Guutuu</th><th className="text-right">Hordoffii Galmee</th></tr>
                </thead>
                <tbody className="divide-y divide-slate-800 text-slate-300">
                  {attendanceRecords.map((s, idx) => (
                    <tr key={idx} className="hover:bg-slate-900/20">
                      ...
                      <td className="text-right">
                        <select value={s.status || 'Not Marked'} onChange={(e) => handleAttendanceCellChange(s.studentId, e.target.value)} className={`bg-[#0f172a] border rounded p-1 text-[11px] font-bold outline-none \${s.status === 'Present' ? 'border-emerald-800 text-emerald-400' : s.status === 'Absent' ? 'border-red-800 text-red-400' : 'border-slate-800 text-slate-400'}`}><option value="Not Marked">Not Marked</option><option value="Present">Present</option><option value="Absent">Absent</option></select>
                      </td>
                    </tr>
                  ))}
                </tbody>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 font-mono text-xs">
            <section className="bg-[#1e293b] p-4 rounded-lg border border-slate-800 space-y-3">
              ...
              <form onSubmit={handleLibrarySubmit} className="space-y-2">
                <input type="text" placeholder="Resource Title" value={libraryForm.title} onChange={e => setLibraryForm({...libraryForm, title: e.target.value})} className="w-full bg-[#0f172a] border border-slate-800 p-2 text-white outline-none rounded" required />
                <input type="text" placeholder="Author Name" value={libraryForm.author} onChange={e => setLibraryForm({...libraryForm, author: e.target.value})} className="w-full bg-[#0f172a] border border-slate-800 p-2 text-white outline-none rounded" required />
                <input type="text" placeholder="Asset Download Link URL" value={libraryForm.downloadUrl} onChange={e => setLibraryForm({...libraryForm, downloadUrl: e.target.value})} className="w-full bg-[#0f172a] border border-slate-800 p-2 text-white outline-none rounded" required />
            ...
            <section className="lg:col-span-2 bg-[#1e293b] p-4 rounded-lg border border-slate-800">
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-left">
                  <thead>
                    <tr className="border-b border-slate-800 text-slate-400 text-[10px] uppercase"><th className="pb-2">Title</th><th>Author</th><th>Section</th><th className="text-right">Access</th></tr>
                  </thead>
                  <tbody className="divide-y divide-slate-800 text-slate-300">
                    {books.map((b, idx) => (
                      <tr key={idx} className="hover:bg-slate-900/20"><td className="py-2.5 font-semibold text-slate-200">{b.title}</td><td>{b.author}</td><td>{b.grade_section}</td><td className="text-right"><a href={b.download_url} target="_blank" rel="noreferrer" className="text-blue-400 underline font-bold">Download 📥</a></td></tr>
                    ))}
                  </tbody>
                </table>
              </div>
        ...
        {activeTab === 'student-transcript' && (
          <section className="bg-[#1e293b] p-5 rounded-xl border border-slate-800 font-mono text-xs max-w-3xl mx-auto space-y-4">
            <div className="flex justify-between items-center border-b border-slate-800 pb-3">
              <div><h3 className="text-sm font-bold text-white uppercase">Personal Academic Registry Card</h3></div>
              <select value={selectedGrade} onChange={(e) => setSelectedGrade(e.target.value)} className="bg-[#0f172a] border border-slate-800 p-1 rounded text-white outline-none"><option value="12 Natural">12 Natural</option><option value="12 Social">12 Social</option><option value="10">10</option><option value="9">9</option></select>
            </div>
            <div className="overflow-x-auto">
              <table className="w-full text-left whitespace-nowrap">
                <thead>
                  <tr className="text-slate-400 text-[10px] uppercase border-b border-slate-800"><th className="pb-2">Student ID</th><th>Full Name</th><th>Test 1</th><th>Test 2</th><th>Assignment</th><th>Final Exam</th><th className="text-right">Total Score</th></tr>
                </thead>
                <tbody className="text-slate-200 divide-y divide-slate-800">
                  {students.map((s, idx) => (
                    <tr key={idx}><td className="py-3 font-bold text-amber-400">{s.studentId}</td><td className="font-semibold">{s.name}</td><td>{s.test1 || 0} / 10</td><td>{s.test2 || 0} / 10</td><td>{s.assignment || 0} / 20</td><td>{s.finalExam || 0} / 60</td><td className="text-right font-black text-emerald-400 text-sm">{s.totalScore || 0} / 100</td></tr>
                  ))}
                </tbody>
              </table>
            </div>
        ...
        {activeTab === 'student-exams' && (
          <section className="bg-[#1e293b] p-4 rounded-lg border border-slate-800 max-w-xl mx-auto font-mono text-xs">
            ...
            <div className="space-y-2">
              ...
              {exams.map((ex, idx) => (
                <div key={idx} className="p-3 bg-[#0f172a] border border-slate-800 rounded-xl flex justify-between items-center">
                  ...
                  <button onClick={async () => { const response = await fetch(`/api/exams?examId=\${ex.exam_id}`); const dbData = await response.json(); if (dbData.success && dbData.questions && dbData.questions.length > 0) { setActiveQuizExam(ex); setQuizQuestions(dbData.questions); setStudentAnswers({}); setStudentExId(''); } else { alert("Questions matrix empty."); } }} className="px-3 py-1 bg-amber-600 hover:bg-amber-500 font-bold text-black text-[10px] rounded uppercase">Take Test 📝</button>
                </div>
              ))}
            </div>
          </section>
        ...
        {activeTab === 'student-library' && (
          <section className="bg-[#1e293b] p-4 rounded-lg border border-slate-800 max-w-2xl mx-auto font-mono text-xs">
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-left whitespace-nowrap">
                <thead>
                  <tr className="border-b border-slate-800 text-slate-400 text-[10px] uppercase"><th>Resource Title</th><th>Author Course Route</th><th className="text-right">Action Handle</th></tr>
                </thead>
                <tbody className="divide-y divide-slate-800 text-slate-300">
                  {books.map((b, idx) => (
                    <tr key={idx}><td className="py-2.5 font-semibold text-slate-200">{b.title}</td><td>{b.author}</td><td className="text-right"><a href={b.download_url} target="_blank" rel="noreferrer" className="text-amber-400 font-bold underline">Download Resource 📥</a></td></tr>
                  ))}
                </tbody>
              </table>
            </div>
      ...
      <div className="fixed bottom-6 right-6 z-50">
        <div className="bg-[#1e293b] border border-slate-800 shadow-2xl rounded-xl p-4 w-72 space-y-3 font-mono text-xs">
          <div className="flex justify-between items-center border-b border-slate-700 pb-1.5"><span className="font-bold text-emerald-400 animate-pulse">● Gargaaraa AI Dijitaalaa</span><span className="text-[10px] text-slate-500 uppercase tracking-widest">v3.3 Node</span></div>
          ...
          <div className="flex gap-1.5">
            <input type="text" placeholder="Gaaffii kee asitti barreessi..." onKeyDown={async (e) => { if (e.key === 'Enter' && e.target.value.trim()) { const txt = e.target.value; e.target.value = ''; const log = document.getElementById('aiTerminalChatLog'); log.innerHTML += `<p class="text-blue-400 font-bold mt-1">Isin:</p><p class="text-slate-200">\${txt}</p>`; const res = await fetch('/api/ai-chat', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ message: txt, userRole: 'Admin' }) }); const d = await res.json(); log.innerHTML += `<p class="text-emerald-400 font-bold mt-1">Gargaaraa AI:</p><p class="text-slate-300">\${d.reply}</p>`; log.scrollTop = log.scrollHeight; } }} className="w-full bg-[#0f172a] border border-slate-800 rounded p-1.5 text-white outline-none text-[11px]" />
          </div>
        </div>
      </div>
      ...
      {activeQuizExam && (
        <div className="fixed inset-0 bg-black/80 flex items-center justify-center p-4 z-50 overflow-y-auto font-mono text-xs text-slate-100">
          <div className="bg-[#141b2d] border border-slate-800 rounded-xl max-w-2xl w-full p-6 space-y-4 shadow-2xl">
            ...
            <button onClick={async () => { if (!studentExId) return alert("Please specify a valid Student ID."); const res = await fetch('/api/exams/submit', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ studentId: studentExId, examId: activeQuizExam.exam_id, answers: studentAnswers }) }); const data = await res.json(); if (res.ok) { alert(`Exam grading complete! Result Output: \${data.score}%`); setActiveQuizExam(null); setStudentAnswers({}); setStudentExId(''); fetchLiveRosterData(); } }} className="w-full bg-emerald-600 py-2.5 text-white font-bold uppercase rounded text-xs">Submit Test 🚀</button>
          </div>
        </div>
      )}
    </div>
